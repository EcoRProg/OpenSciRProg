...
range(fixedChla$IntegE1, na.rm = TRUE) #na.rm remueve los valores NA de la variable 
...
ChlaByMonth <- ChlaRearrenged %>% filter(!is.na(Month)) %>% group_by(Year, Month) %>% summarise(ChlaInteg=mean(ChlaInteg, na.rm = TRUE)) #Del data frame ChlaRearrenged, agrupa por año y mes y calcula la media de la variable ChlaInteg para cada mes de cada año

ggplot(ChlaByMonth, aes(factor(Month), ChlaInteg, color=factor(Year))) + theme_minimal() + scale_y_log10() + geom_point() + geom_line()

ggplot(ChlaByMonth, aes(Year, ChlaInteg, color=factor(Month))) + theme_minimal() + scale_y_log10() + geom_point() + geom_line()

ggplot(ChlaByMonth, aes(Year, ChlaInteg, color=factor(Month))) + theme_minimal() + scale_y_log10() + geom_point() + geom_line() + facet_wrap(~Month)
